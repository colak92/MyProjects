...
public interface ExamRepository extends JpaRepository<Exam, Integer>{
	...
	@Query(value = "SELECT * FROM exams e, examperiods ep, subjects s WHERE e.examperiod_id = ep.id AND e.subject_id = s.id AND e.examperiod_id = :examperiodid AND e.subject_id = :subjectid", nativeQuery = true)
	public List<Exam> checkExamSubjectExists(@Param("examperiodid") Integer examperiodid, @Param("subjectid") Integer subjectid);
