...
public class ExamService {
	...
	public ResponseEntity <List<ExamDTO>> getExamsForStudent(int userID) {
		Optional<User> u = userRepository.findById(userID);
		Student s = studentRepository.findStudentByEmail(u.get().getEmail());
		
		List<ExamDTO> studentExams = examMapper.toExamDTOs(examRepository.checkStudentHasExam(s.getId()));
		return ResponseEntity.ok(studentExams);
	}
	
	public ResponseEntity <List<ExamDTO>> getExamsForProfessor(int userID) {
		Optional<User> u = userRepository.findById(userID);
		Professor p = professorRepository.findProfessorByEmail(u.get().getEmail());
		
		List<ExamDTO> professorExams = examMapper.toExamDTOs(examRepository.checkProfessorHasExam(p.getId()));
